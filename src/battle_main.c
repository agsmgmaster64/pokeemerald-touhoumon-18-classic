...
const u8 gTypeEffectiveness[336] =
{
    ...
    TYPE_BEAST, TYPE_STEEL, TYPE_MUL_NOT_EFFECTIVE,
...
u8 IsRunningFromBattleImpossible(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (side != GetBattlerSide(i)
         && gBattleMons[i].ability == ABILITY_OLD_SHADOW_TAG)
        {
    ...
    i = AbilityBattleEffects(ABILITYEFFECT_CHECK_FIELD_EXCEPT_BATTLER, gActiveBattler, ABILITY_OLD_MAGNET_PULL, 0, 0);
            ...
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF << 28) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 12))))
            {
                ...
                case B_ACTION_SWITCH:
                    *(gBattleStruct->battlerPartyIndexes + gActiveBattler) = gBattlerPartyIndexes[gActiveBattler];
                    ...
                    else if ((i = ABILITY_ON_OPPOSING_FIELD(gActiveBattler, ABILITY_OLD_SHADOW_TAG))
